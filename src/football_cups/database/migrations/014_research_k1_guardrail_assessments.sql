CREATE TABLE research.k1_guardrail_assessments (
    record_id text PRIMARY KEY REFERENCES research.records(record_id) ON DELETE CASCADE,
    prediction_record_id text NOT NULL REFERENCES research.shadow_predictions(record_id),
    channel text NOT NULL,
    fixture_id text NOT NULL,
    competition_id text NOT NULL CHECK (competition_id = '16'),
    ...
    historical_dataset_sha256 text NOT NULL CHECK (historical_dataset_sha256 ~ '^[0-9a-f]{64}$'),
    git_commit text,
    ...
    UNIQUE (prediction_record_id, policy_version)
);

CREATE INDEX research_k1_guardrail_fixture_idx
    ON research.k1_guardrail_assessments(fixture_id, target, prediction_cutoff);

CREATE VIEW research.current_k1_guardrail_assessments AS
SELECT DISTINCT ON (prediction_record_id) assessment.*
FROM research.k1_guardrail_assessments AS assessment
ORDER BY prediction_record_id, policy_revision DESC, assessed_at DESC, record_id DESC;
