...
欧战护栏同时读取`config/research-europe-guardrail.json`，但独立于模型激活与K1 assessment。任务先执行原`shadow-predict`，再执行`europe-guardrail-shadow`，两步成功后导入一次；任一步非零即退出并保留输出。欧冠`101`和欧罗巴`63`的自然窗口只追加拟议动作；源代码或策略相关路径未提交时只追加`unavailable/relevant_source_not_reproducible`，不得补发、修改基础概率或绕过策略生效时间。
