...
- 十二个版本化迁移真实执行；`003_automated_results` 增加自动赛果证据，`004/005` 建立隔离研究层，`006/007` 添加并启用盘口标准化 V2，`008` 启用无效 fixture 逻辑排除，`009` 增加中国体彩官方 90 分钟赛果证据，`010` 增加清单失败分类和映射身份溯源，`011` 增加负责人声明字段和候选引用资格，`012` 增加隔离影子预测表和研究 flag V2 约束。
