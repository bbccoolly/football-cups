...
- [x] 记录 D-030，新增版本化赛事类型与市场证据等级；9个当前赛事ID全部显式登记，未知或分层冲突赛事固定 abstain，不再按名称猜测。
- [x] 影子发布改为使用身份自身预测截止时间前可见的 `FixtureIdentity`，保存注册表文件/canonical双哈希、公司概率MAD、方向强度、置信上限和风险标签；赛事等级不修改概率。
- [x] 完成迁移013、研究 importer 和分层评估；旧预测保持不可变并按 `legacy_unclassified` 报告，人工赛果不计入解除 high 置信的同赛事/同切点200场门禁。
...
截至 2026-07-21 11:20 Asia/Shanghai 的赛事分层上线证据：

- 迁移013已应用；9个当前赛事ID全部精确命中：欧冠/欧罗巴/世界杯为A，韩职/美职足/瑞超/挪超/巴甲为B，芬超为C，0个冲突或未登记。
- 赛事注册表使用 `competition-profile-v1` 和 `shadow-confidence-v1`；全部初始分类为 `provisional`，因此当前任何赛事均不能产生 high 置信，芬超最高为 low。
- 旧版10条影子预测涉及4个fixture，保持不可覆盖并按 `legacy_unclassified` 评估；10条均已有自动验证赛果，小样本 Log Loss 为1.1828、方向命中率40%，不能形成校准结论。
- 新版真实当前时间 dry-run 为0个自然发布机会，没有历史补发；下一条自然机会将首次携带as-of身份、双哈希、等级、置信和风险字段。
- 研究评估首次导入新增1条，重复导入新增0；`research.shadow_predictions=10`、`shadow_evaluations=3`，正式严格切点计数保持 `T-48h=8`、`T-24h=29`、`T-12h=34`、其余主要切点45。
- 离线测试133项通过、2项按配置跳过；使用专用 `football_cups_test` 的真实PostgreSQL全量测试135项全部通过。
- G盘镜像批次 `20260721T030747716161Z-aef425d7` 完成9172个源文件核对，内容寻址批次 `20260721T030759163416Z-d0897ed0` 完成4588个新对象、复用4584个对象，SQLite检查正常。
- 增量备份开始前，正在运行的影子任务已自动应用纯追加迁移013；随后双层备份完整覆盖迁移后事实层。未发生记录删除、预测重发或正式资格计数变化。
- `health=ok`、SQLite检查正常、0个逾期任务；采集、数据库导入和影子任务最近一次均返回0。
...
- 影子预测任务持续返回0；已有10条迁移013前自然发布记录并完成赛果评估，但尚未产生携带赛事分层字段的新版自然发布。当前4场/10条仍远不足以评价分层概率稳定性。
...
观察 D-030 上线后的首个真实影子发布窗口：若自然切点内存在 live V2 合格欧赔快照，核验其只使用截止前身份并携带赛事等级、双哈希、置信和风险字段；若无窗口则保持 `unchanged`，不得历史补发。同时继续低频体彩官方补偿，不扩大访问频率；阿里云 ECS 保持所有 timer 禁用。
