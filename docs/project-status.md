...
保持采集和数据库导入任务连续运行，完成 7 天验证并从修复版上线时间重新累计 30 天赛果稳定性窗口。持续积累自动候选、已验证赛果和隔离证据。阶段 4 仍需按切点至少 500 个不同 fixture 同时具有严格快照及唯一有效赛果；D-022 只允许离线历史研究，当前不得开始正式模型。
...
- 采集和两个备份 S4U 任务、真实备份批次恢复及专用非管理员数据库任务自动启动均已通过；注销 10 分钟和重启 Windows 完成前，30 天无人值守门禁保持未通过。
...
- [ ] 安排一次 Windows 重启，确认 G 盘映射、PostgreSQL 自动启动、四个无人值守任务和采集心跳恢复。

## Agent 唯一下一步

完成注销 Windows 用户至少 10 分钟的无人值守验证，再安排一次 Windows 重启验证；这两项需要项目负责人选择可中断当前会话的时间窗口。期间继续 7 天技术验证、从 2026-07-17 10:44:44 Asia/Shanghai 开始的 30 天赛果窗口和每日 3 场人工核验。阿里云 ECS 保持所有 timer 禁用。
