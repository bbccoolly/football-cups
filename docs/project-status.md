...
- [x] 将采集、数据库导入、每日备份和每周备份四个任务注册为 S4U，在 PostgreSQL 已运行时逐项手工触发均返回 0。
...
截至 2026-07-17 12:12 Asia/Shanghai：
...
- 最新 `health=ok`：每日与每周备份状态均为 `ok`，G 盘备份可用空间约 457 GB。四个任务均为 `S4U`、`Limited` 和 `IgnoreNew`，手工触发返回 0。
...
- 停止 PostgreSQL 后执行数据库 S4U 任务，确认 `lcz` 虽为 `RunLevel=Limited` 仍携带管理员组身份，PostgreSQL 明确拒绝启动。已恢复非提升会话中的 PostgreSQL；新增专用非管理员任务用户脚本，但第二次 UAC 被取消，尚未应用。
...
- 采集和两个备份 S4U 任务已通过，真实备份批次恢复已通过；数据库任务仍绑定管理员组用户 `lcz`，只能在 PostgreSQL 已运行时导入。专用非管理员账户配置、注销 10 分钟和重启 Windows 完成前，30 天无人值守门禁保持未通过。
...
- [x] 在提升的 PowerShell 中把采集、数据库导入、每日备份和每周备份四个任务注册为 S4U。
- [ ] 批准管理员脚本创建 `football-cups-runner`，将数据库任务改绑专用非管理员 S4U 账户，并验证停止 PostgreSQL 后能够自动启动和导入。
- [ ] 注销 Windows 用户至少 10 分钟，重新登录后确认采集至少两轮、数据库至少一轮且 `health=ok`。
- [ ] 安排一次 Windows 重启，确认 G 盘映射、PostgreSQL 自动启动、四个 S4U 任务和采集心跳恢复。

## Agent 唯一下一步

批准并运行专用非管理员数据库任务账户配置，将 `FootballCups-Database-Import` 改绑该账户并验证停止 PostgreSQL 后可自动启动；随后完成注销 10 分钟和 Windows 重启验证。期间继续 7 天技术验证、从 2026-07-17 10:44:44 Asia/Shanghai 开始的 30 天赛果窗口和每日 3 场人工核验。阿里云 ECS 保持所有 timer 禁用。
