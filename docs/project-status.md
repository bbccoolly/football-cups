...
- [x] Windows 备份改动通过 72 项测试，包含真实 PostgreSQL 集成测试和 PowerShell 语法检查。
- [x] 将采集、数据库导入、每日备份和每周备份四个任务注册为 S4U，逐项手工触发均返回 0。
...
- 四个 S4U 任务及真实批次恢复已通过；尚未实际注销 10 分钟或重启 Windows，这两个外部运行验证完成前，30 天无人值守门禁保持未通过。
...
完成注销 10 分钟和 Windows 重启验证，确认 G 盘映射、PostgreSQL、四个 S4U 任务和采集心跳可以自动恢复。期间继续 7 天技术验证、从 2026-07-17 10:44:44 Asia/Shanghai 开始的 30 天赛果窗口和每日 3 场人工核验。阿里云 ECS 保持所有 timer 禁用。
